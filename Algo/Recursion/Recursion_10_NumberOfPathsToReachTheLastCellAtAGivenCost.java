...
*/
class Recursion_10_NumberOfPathsToReachTheLastCellAtAGivenCost {
    ...
    static int numberOfPathsToReachLastCellUtil(int[][] cost, int m, int n, int i, int j, int costAvailable) {
        ...
        // Choice 1
        int traverseRightCell = numberOfPathsToReachLastCellUtil(cost, m, n, i, j + 1, costAvailable - cost[i][j]);

        // Choice 2
        int traverseBottomCell = numberOfPathsToReachLastCellUtil(cost, m, n, i + 1, j, costAvailable - cost[i][j]);

        return traverseRightCell + traverseBottomCell;
    }

    static int numberOfPathsToReachLastCell(int[][] cost, int m, int n, int minCost) {
        return numberOfPathsToReachLastCellUtil(cost, m - 1, n - 1, 0, 0, minCost);
    }
